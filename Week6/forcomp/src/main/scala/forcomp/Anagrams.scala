...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(c => c).map({ case (k, o) => k -> o.length }).toList.sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case x :: xs =>
      ...
      combs ++ (for (c <- combs; i <- 1 to x._2) yield (x._1, i) :: c)
  }
  ...
  def subtract(xs: Occurrences, ys: Occurrences): Occurrences = {
    val ysMap = ys.toMap withDefaultValue 0
    xs.reverse.foldLeft(List[(Char, Int)]()) {
      case (acc, (c, i)) =>
        val sub = i - ysMap(c)
        if (sub > 0) (c, sub) :: acc
        else acc
    ...
    def innerSentenceAnagrams(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case _ =>
        for {
          ...
          sentence <- innerSentenceAnagrams(subtract(occurrences, combination))
        } yield word :: sentence
    }
